...
activity. For example, when applied to a stream of user clicks, a session window
with a 30 minute gap aggregates all clicks that occur within 30 minutes of one
another. Once there is a 30 minute gap for a given user, the session window
closes and a new one opens with the next click.
